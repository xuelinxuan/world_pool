...
market_history_currency_callable =market_currency(extract_cb_market_history_raw, extract_cb_currency_history_raw)

extract_cb_market_daily_raw      =S3.extract(cb_market_daily_raw)
extract_cb_currency_daily_raw    =S3.extract(cb_currency_daily_raw)
market_daily_currency_callable   =market_currency(extract_cb_market_daily_raw, extract_cb_currency_daily_raw)
...
with DAG(dag_id='market_pv',   schedule_interval=None, start_date=datetime(2023, 1, 1),  catchup=False) as dag:
    ...
    market_history_currency           = PythonOperator(task_id='market_history_currency',         python_callable = market_history_currency_callable,provide_context = True)
    ...
    market_daily_currency             = PythonOperator(task_id='market_daily_currency',           python_callable = market_daily_currency_callable,provide_context = True)
